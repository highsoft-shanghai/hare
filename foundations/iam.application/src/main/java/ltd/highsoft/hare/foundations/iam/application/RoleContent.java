...
public record RoleContent(Id id, Name name, Set<String> authorities, Remarks remarks) {

    public static RoleContent build(Role role) {
        return new RoleContent(role.id().id(), role.name(), role.grantedAuthorities().asSet(), role.remarks());
    }
